use crate::dto::create_dto::{CreatePostDTO, HistoryDTO, PostDetailsDTO, PostIdQuery};
...
pub async fn create_post(
    ...
    let image_name = match process_result {
        Ok(Ok(image_name)) => {
            println!(
                "Image successfully processed in background and saved to {}",
                image_name
            );
            image_name
        }
...
fn process_image(
    ...
    let sticker = image::imageops::resize(
        &sticker,
        width,
        height,
